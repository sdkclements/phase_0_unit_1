...
window.Calendar = {
  ...
  setup: function() {
    ...
    $('.prev').on('click', function(e) {
      ...
      $calendar = $('.calendar');
      $calendar.hide("drop", { direction: "right" }, 300, function() {
        date = Calendar.displayedDate
        date.setDate(date.getDate() - 1);
        $calendar.show("drop", { direction: "left" }, 300, function badFunction(){
    ...
    $('.next').on('click', function(e) {
      $('.create input[type="text"]').removeClass('required');
      $calendar = $('.calendar');
      $calendar.hide("drop", { direction: "left" }, 300, function() {
        date = Calendar.displayedDate
        date.setDate(date.getDate() + 1);
        $calendar.show("drop", { direction: "right" }, 300, function(){
          Calendar.displayedDate = date;
          Calendar.displayDate();
        });
        e.preventDefault();
